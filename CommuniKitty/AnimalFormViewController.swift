...
class AnimalFormViewController : FormViewController, FusumaDelegate, CLImageEditorDelegate {
    ...
    func loadTraits() {
        if let animal = self.animalObject {
            var traitsArray = Set<String>()
            if let traits = animal["traits"] as? [[String:Any]], (traits.count > 0) {
                for trait in traits {
                    if let name = trait["name"] as? String {
                        traitsArray.insert(name)
                    }
                }
                
            }
            self.selectedTraitStrings = traitsArray
            self.form.rowBy(tag: self.TRAITS_TAG)?.baseValue = traitsArray
    ...
    func loadLoves(){
        if let animal = self.animalObject{
            ...
            if let loves = animal["loves"] as? [[String:Any]]{
                if loves.count > 0 {
                    for love in loves {
                        value.insert(love["text"] as? String ?? "")
                    }
                }
            }
...
func loadHates(){
    if let animal = self.animalObject{
            ...
            var value = Set<String>()
            if let loves = animal["hates"] as? [[String:Any]]{
                if loves.count > 0 {
                    for love in loves {
                        value.insert(love["text"] as? String ?? "")
                    }
...
func generateForm() {
    ...
    form +++ Section("Info") { section  in
        ...
        <<< NameRow(NAME_TAG) {
            $0.title = "Name"
            if self.isEditMode() {
                $0.value = self.animalObject?["name"] as? String
            }
        }.cellSetup { cell, row in
        ...
        <<< TwitterRow(USERNAME_TAG) {
            $0.title = "Username"
            if self.isEditMode() {
                $0.value = self.animalObject?["username"] as? String
            }
        }.cellSetup { cell, row in
        ...
        <<< SegmentedRow<String>(GENDER_TAG) {
            ...
            if self.isEditMode() {
                $0.value = self.animalObject?["gender"] as? String
            }
        }.cellSetup { cell, row in
        ...
        <<< DateInlineRow(BIRTHDATE_TAG) {
            ...
            if self.isEditMode() {
                let dateStr = self.animalObject?["birthDate"] as? String
                let dateFormatter = DateFormatter()
                dateFormatter.dateFormat = "yyyy-MM-dd"
                $0.value = dateFormatter.date(from: dateStr ?? "")
            }
        }.cellSetup { cell, row in
        ...
        <<< GooglePlacesTableRow(HOMETOWN_TAG) {
            ...
            if self.isEditMode() {
                if let hometownVal = (self.animalObject?["location"]as? [String: Any])?["name"] as? String {
        ...
        <<< TextAreaRow(INTRO_TAG) {
            ...
            if self.isEditMode() {
                $0.value = self.animalObject?["intro"] as? String
            }
        }.cellSetup { cell, row in
    ...
    form +++ Section("Soshul Meowdia") { section  in
        ...
        <<< TwitterRow(TWITTER_TAG) {
            $0.title = "Twitter"
            if self.isEditMode() {
                $0.value = self.animalObject?["twitterUsername"] as? String
            }
        }.cellSetup { cell, row in
            cell.textField.placeholder = "@username"
        ...
        <<< TwitterRow(YOUTUBE_TAG) {
            $0.title = "Youtube"
            if self.isEditMode() {
                $0.value = self.animalObject?["youtubeUsername"] as? String
            }
        }.cellSetup { cell, row in
            cell.textField.placeholder = "@username"
        ...
        <<< TwitterRow(FACEBOOK_TAG) {
            $0.title = "Facebook"
            if self.isEditMode() {
                $0.value = self.animalObject?["facebookPageName"] as? String
            }
        }.cellSetup { cell, row in
        ...
        <<< TwitterRow(INSTAGRAM_TAG) {
            $0.title = "Instagram"
            if self.isEditMode() {
                $0.value = self.animalObject?["instagramUsername"] as? String
            }
        }.cellSetup { cell, row in
            cell.textField.placeholder = "@username"
